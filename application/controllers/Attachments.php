...
class Attachments extends Admin_Controller
{
    ...
    public function index()
    {
        ...
        $this->data['lmsGrades'] = $this->db->get('lms_grades')->result();
        $this->data['lmsSubjects'] = $this->db->get('lms_subjects')->result();
    ...
    public function save()
    {
        ...
        if ($_POST) {
            ...
            $this->form_validation->set_rules('file_name', translate('file_name'), 'trim|required');
            $this->form_validation->set_rules('enc_name', translate('enc_name'), 'trim|required');
            if (isset($_POST['branch_id'])) {
                if ($_POST['branch_id'] == 'all_branches') {
                    $this->form_validation->set_rules('type_id', translate('type'), 'trim');
                    ...
                    $this->form_validation->set_rules('lms_grade_id', translate('LMS grade'), 'trim|required');
                    $this->form_validation->set_rules('lms_subject_id', translate('LMS subject'), 'trim|required');
                } else {
                    ...
                    $this->form_validation->set_rules('lms_grade_id', translate('LMS grade'), 'trim');
                    $this->form_validation->set_rules('lms_subject_id', translate('LMS subject'), 'trim');
                }
            }
    ...
    {
        $file_name = 'uploads/attachments/' . $_POST['enc_name'];
    ...
    public function upload_file()
    {
        ...
        $allowedSize = floatval(30000000 * $allowedSizeKB);
        ...
        if (isset($_FILES["attachment_file"]) && !empty($_FILES['attachment_file']['name'])) {
            ...
            if ($file_size > 0) {
                ...
                if (!in_array(strtolower($extension), $allowedExts)) {
                    echo json_encode(['status' => 'error', 'message' => translate('this_file_type_is_not_allowed')]);
                    return;
                }

                if ($file_size > $allowedSize) {
                    echo json_encode(['status' => 'error', 'message' => translate('file_size_shoud_be_less_than') . " $allowedSizeKB KB."]);
                    return;
                }

                $config['upload_path'] = 'uploads/attachments/'; // Change this to your desired file upload path
                ...
                if ($this->upload->do_upload('attachment_file')) {
                    ...
                    echo json_encode(['status' => 'success', 'file_name' => $this->upload->data('orig_name'), 'enc_name' => $this->upload->data('file_name')]);
                    return;
                } else {
                ...
                echo json_encode(['status' => 'error', 'message' => translate('error_reading_the_file')]);
                return;
            }
        } else {
            if (isset($_POST['attachment_id'])) {
                echo json_encode(['status' => 'success', 'file_name' => '']);
                return;
            } else {
                echo json_encode(['status' => 'error', 'message' => 'The Attachment field is required.']);
                return;
            }
        }
    }
