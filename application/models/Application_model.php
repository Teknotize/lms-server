...
class Application_model extends CI_Model
{
    ...
    {
        $this->db->select('e.*, s.photo, CONCAT_WS(" ", s.first_name, s.last_name) as fullname, s.register_no, s.parent_id, s.email, s.mobileno, s.blood_group, s.birthday, s.admission_date, c.name as class_name, se.name as section_name, sc.name as category');
        $this->db->from('enroll e');
        $this->db->join('student s', 'e.student_id = s.id');
        // $this->db->join('login_credential l', 'l.user_id = s.id AND l.role = 7');
    ...
    public function getStudentListByClassSection_old($classID = '', $sectionID = '', $branchID = '', $deactivate = false, $rollOrder = false)
    {
