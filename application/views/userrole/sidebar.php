                    ...
                    if (is_student_loggedin()) { ?>
                        ...
                        <li class="nav-parent <?php if ($main_menu == 'digital_library') echo 'nav-expanded nav-active'; ?>">
                            ...
                            <ul class="nav nav-children">
                                ...
                                $enroll = $this->db->where('student_id', $student_id)->get('enroll')->row();

                                $this->db->select('subject.id, subject.name, attachments.title'); // Selects all columns from both tables
                                $this->db->from('subject');
                                $this->db->join('attachments', 'subject.id = attachments.subject_id', 'inner');
                                $this->db->where('attachments.branch_id', $enroll->branch_id);
                                $this->db->where('attachments.class_id', $enroll->class_id);
                                $this->db->group_by('subject.id');

                                $subjects = $this->db->get('class')->result();

                                //$grades = $this->db->get('class')->result();
                                ?>

                                <?php foreach ($subjects as $subject) { ?>
                                    <li class="<?php if ($subject_id == $subject->id) echo 'nav-active'; ?>">
                                        <a href="<?= base_url('digital_library/subject/' . $enroll->branch_id . '/' . $enroll->class_id.'/'.$subject->id) ?>">
                                            <span>
                    ...
                    ?>
                        ...
                        <li class="<?php if ($main_menu == 'attachments') echo 'nav-active'; ?> ">
                            <a href="<?= base_url('userrole/attachments') ?>">
                                <i class="icons icon-cloud-upload"></i><span><?= translate('content') ?></span>
                            </a>
                        </li>
